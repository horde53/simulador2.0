document.addEventListener('DOMContentLoaded', function() {
    ...
    camposMoeda.forEach(campo => {
        $(campo).maskMoney({
            ...
            allowZero: true,
            allowNegative: false,
            rightAlign: false, // Alinhamento à esquerda para melhor experiência mobile
            selectAllOnFocus: true // Seleciona todo o conteúdo ao focar, facilitando a edição
        });
    });
    
    // Inicializar os campos monetários com valor zero
    camposMoeda.forEach(campo => {
        $(campo).maskMoney('mask', 0);
        
        // Adicionar evento de foco para melhorar a experiência no mobile
        $(campo).on('focus', function() {
            // Pequeno atraso para garantir que o campo esteja pronto
            setTimeout(() => {
        ...
        // Corrigir comportamento no mobile
        $(campo).on('keyup', function(e) {
            // Se o usuário pressionar backspace ou delete
            if (e.keyCode === 8 || e.keyCode === 46) {
                const valorAtual = $(this).val().replace(/[^\d,]/g, '');
    ...
    function validarFormulario() {
        ...
        camposObrigatorios.forEach(campo => {
            ...
            if (!valor) {
                ...
                if (['currentRent', 'familyIncome', 'propertyValue', 'downPayment', 'fgts'].includes(campo)) {

...
function carregarEstatisticas() {
    fetch('/api/simulacoes?pagina=1&limite=1000')
        ...
        .then(data => {
            ...
            // Clientes únicos
            const clientesUnicos = new Set(data.simulacoes.map(sim => sim.cliente.nome)).size;
            document.getElementById('clientes-unicos').textContent = clientesUnicos;
            
            // Média do valor dos imóveis
            if (data.simulacoes.length > 0) {
                const somaValores = data.simulacoes.reduce((acc, sim) => acc + (sim.valorImovel || sim.valorCredito), 0);
    ...
    fetch(`/api/simulacoes?pagina=${pagina}&limite=${limite}`)
        ...
        .then(data => {
            ...
            data.simulacoes.forEach(simulacao => {
                ...
                // Formatar data
                const data = new Date(simulacao.dataCriacao);
                ...
                row.innerHTML = `
                    ...
                    <td>${formatarMoeda(simulacao.valorImovel || simulacao.valorCredito)}</td>
                    <td>${formatarMoeda(simulacao.cliente.renda)}</td>
                    <td class="action-buttons">
                        <button class="btn btn-info btn-view" data-id="${simulacao.id}">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button class="btn btn-danger btn-delete" data-id="${simulacao.id}">
